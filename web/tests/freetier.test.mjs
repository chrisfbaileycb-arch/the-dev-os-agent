...
import { FREE_MODELS, XKIRO_DEFAULT_BASE, createBurstLimiter, creditsForTokens, freeModel, freeModels, freeTierStatus, fundedModels, isFrontier, routeFreeRequest, xkiroBase, xkiroPool } from '../server/freetier.mjs';
...
test('the deployment never funds a frontier model from its own key', () => {
  ...
  for (const id of ['anthropic/claude-3.5-sonnet', 'openai/gpt-4o', 'openai/gpt-5', 'deepseek/deepseek-r1', 'openai/o3-mini', 'x-ai/grok-4', 'google/gemini-2.5-pro', 'qwen/qwq-32b-reasoning']) {
    ...
    assert.equal(freeModel(id, { XKIRO_FREE_MODELS: id, XKIRO_API_KEY: 'k' }), undefined, `${id} must not be fundable`);
  }
  for (const m of FREE_MODELS) assert.equal(isFrontier(m.id), false, `${m.id} is shipped as free`);
  // The escape hatch is one explicit variable, so an operator whose plan covers it can opt in.
  assert.equal(freeModel('deepseek/deepseek-r1', { FREE_TIER_ALLOW_FRONTIER: 'true' })?.provider, 'xkiro');
  assert.ok(xkiroPool({}).includes('deepseek/deepseek-r1'), 'the gateway pool still names it');
  assert.ok(!freeModels({}).some(m => m.id === 'deepseek/deepseek-r1'), 'but this deployment does not fund it');
});
...
test('the funded list says which provider serves each model', () => {
  ...
  const status = freeTierStatus({ GROQ_API_KEY: 'k', XKIRO_API_KEY: 'k' });
  assert.equal(status.providers['groq/llama-3.3-70b-versatile'], 'groq');
  assert.equal(status.providers['z-ai/glm-5.2'], 'xkiro');
...
test('xKiro funds the free tier on its own, and clears the warming-up state', () => {
  ...
  // deepseek-r1 is in the gateway's pool and deliberately not in this list: the frontier guard
  // drops it, so the tier is funded entirely by cheap models.
  assert.deepEqual(status.models, ['deepseek/deepseek-chat', 'z-ai/glm-5.2', 'z-ai/glm-5.3-flash', 'qwen/qwen-2.5-72b-instruct', 'moonshotai/kimi-k2.7-code']);
...
test('an xKiro stream is metered to the free ledger like any other funded model', async () => {
  ...
  try {
    await withProxy({ env: { XKIRO_API_KEY: 'gateway-key' }, db, transport: async () => stream('data: {"usage":{"total_tokens":4000}}\n\ndata: [DONE]\n\n') }, async url => {
      assert.equal((await chat(url, { provider: 'xkiro', model: 'moonshotai/kimi-k2.7-code', messages: [{ role: 'user', content: 'hi' }] })).status, 200);
      assert.equal(db.usedThisMonth(workspace, new Date(), 'free'), 2, '4,000 tokens at 0.5 credits per 1K');
      assert.equal(db.latestEntry(workspace, 'free').model, 'moonshotai/kimi-k2.7-code');
    });
  } finally { db.close(); }
});

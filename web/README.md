...
- **Dual-service ready.** The backend runs as a public HTTP web service on its own, and optionally alongside a Render Background Worker that claims heavier multi-step runs over the private network and posts results back (`server/jobs.mjs`, `server/worker.mjs`). Deploy the worker and a long workflow stops occupying a dyno that should be streaming SSE; skip it and workflows run in the browser's own Web Worker, which is the default. Queued jobs sit on disk until claimed, so the queue accepts zero-config runs only and refuses anything carrying a key.
...
| Variable | Purpose |
|---|---|
| `GROQ_API_KEY`, `OPENROUTER_API_KEY` | Fund the zero-config free tier. Set either and keyless visitors get live replies; leave both unset and the app opens in the scripted preview. |
...
| `FREE_TIER_DISABLED` | `true` switches the tier off without removing the provider keys. |

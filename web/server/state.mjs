...
export function validateEntry(e) { if (!e || !isId(e.id) || !isIso(e.at) || typeof e.model !== 'string' || e.model.length > 200 || !['free', 'pro'].includes(e.tier) || !['credits', 'byok', 'demo'].includes(e.mode) || !Number.isInteger(e.tokens) || e.tokens < 0 || e.tokens > 10_000_000 || typeof e.credits !== 'number' || !Number.isFinite(e.credits) || e.credits < 0 || e.credits > 1_000_000) throw new HttpError(400, 'Invalid ledger entry.'); }
...
export function createState({ env = process.env, db }) {
  ...
  return async function handler(req, res) {
    ...
    if (path !== '/api/state' && path !== '/api/state/clear') return false;
    try {
      checkOrigin(req, env);
      ...
      if (path === '/api/state' && req.method === 'GET') { json(res, 200, { ...db.state(workspace), pool }); return true; }
      ...
      json(res, 200, { ok: true, used: db.usedThisMonth(workspace), pool }); return true;
    } catch (error) {

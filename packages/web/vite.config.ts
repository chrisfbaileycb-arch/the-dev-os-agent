...
const api: Plugin = { name: 'heybuddy-api', configureServer(server) { const db = openDatabase('data/dev.sqlite'); const handlers = [createProxy({ db }), createState({ db }), createBrowse(), createMcp(), createFetcher(), createGithub(), createJobs({ jobs: openJobs(db) })]; server.middlewares.use((req, res, next) => { void (async () => { for (const handle of handlers) if (await handle(req, res)) return; next(); })().catch(next); }); } };

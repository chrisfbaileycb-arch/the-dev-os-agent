...
describe('streaming provider client', () => {
  ...
  it('loads models through the same-origin proxy', async () => { const mock=vi.fn().mockResolvedValue(new Response(JSON.stringify({data:[{id:'test-model'},{id:2}]}))); vi.stubGlobal('fetch',mock); try { expect(await listModels(message().connection,new AbortController().signal)).toEqual(['test-model']);expect(mock.mock.calls[0][0]).toBe('/api/models'); } finally { vi.unstubAllGlobals(); } });
});
